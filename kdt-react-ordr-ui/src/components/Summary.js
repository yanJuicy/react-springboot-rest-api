...
export function Summary({ items = [] }) {
  ...
  return (
    <>
      ...
      <hr />
      {items.map(v => <SummaryItem key={v.id} count={v.count} productName={v.prodcutName} />)}
      <form>
        <div className="mb-3">
        ...
        <div className="mb-3">
          <label for="address" className="form-label">주소</label>
          <input type="text" className="form-control mb-1" id="address" />
        </div>
        <div className="mb-3">
          <label for="postcode" className="form-label">우편번호</label>
